...
def IPM(A, b, c, detail=False):
    ...
    for k in range(500):
        ...
        def solve(F_d, F_p, F_0):
            ...
            dlam_k, _, _, _ = np.linalg.lstsq(AXS_inv.dot(A.T), -F_p-AXS_inv.dot(F_d)+A.dot(S_invF_0),rcond=-1)
        ...
        while np.linalg.norm(np.multiply(x_k+alpha_p*dx_k,  s_k+alpha_d*ds_k),np.inf)<=gamma*mu:
            # print("search")
            alpha_p *= gamma
            alpha_d *= gamma
        ...
        if detail:
            print(
                ...
                "dot_x={:.5e}".format(
                    dz_k.T.dot(old_dz_k[max(-1, -len(old_dz_k))]).item()
...
if __name__ == "__main__":
    ...
    # # A, b, c, x_star = read_data(r"data/data3")
