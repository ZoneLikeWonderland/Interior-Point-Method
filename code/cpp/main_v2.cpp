...
template <class DataType> class IPM {
    ...
    SparseLU<SMat, COLAMDOrdering<int>> solver_LU;
    SparseQR<SMat, COLAMDOrdering<int>> solver_QR;
    // BiCGSTAB<SMat> solver_BiCG;
    enum SD_type { LLT, LU, QR, LS };
    SD_type solver_type;
  ...
  public:
    ...
    tuple<Mat, Vec> enforce_fullrank(const Mat &A, const Vec &b) {
        ...
        // cout << (A_t.array() == 0).select(99999,
        // A_t.array()).abs().minCoeff()
        //      << "\n";
        return {A_t, b_t};
    }
    ...
    tuple<Vec, Vec, Vec> solve_newton(const Vec &F_d, const Vec &F_p,
        ...
        if (first) {
            ...
            if (solver_LLT.info() == Success) {
                ...
                solver_type = LS;
                //     cout << "LS\n";
                //     // solver_QR.compute(AXS_invAT);
                //     // if (solver_QR.info() == Success) {
                //     //     solver_type = QR;
                //     // } else {
                // cout << "cannot decompose (impossible)"
                //      << " rank(A) = " << JacobiSVD<Mat>(A).rank() << " "
                //      << A.rows() << "x" << A.cols() << "\n";
                // exit(-1);
                //     // }
                // }
